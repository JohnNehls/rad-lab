...
Every estimate coherently integrates the full record before forming the ratio,
so the two domains see the same effective SNR: the time domain sums the samples
(a matched filter), while the frequency domain reads the peak FFT bin, which is
that same coherent sum.  Errors are averaged over Monte Carlo noise trials at
each SNR.

Key takeaways:

- The monopulse ratio gives the same accuracy in time and frequency domains,
  because both coherently integrate the whole record (a time-domain sum equals
  the peak FFT bin).
...
# Matched reference for coherent integration of the RATIO methods: summing each
# element against it integrates the whole record, exactly as the
# frequency-domain peak bin does.  At baseband it is unity (a plain coherent
# sum); at RF it demodulates the tone before summing.
...
for snr_db in snr_db_list:
    ...
    for trial in range(N_TRIALS):
        ...
        # Sum each element against the matched reference (whole-record coherent
        # integration), then form the ratio on the integrated pair.  This is
        # the time-domain equivalent of reading the frequency-domain peak bin.
        ...
        # Uses the RAW (non-demodulated) coherent sum, so at RF the carrier
        # corrupts the phase difference and the estimate fails at passband.
