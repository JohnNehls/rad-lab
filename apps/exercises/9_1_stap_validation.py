...
1. Places a target in the correct range and range-rate bin (clutter-free).
2. Suppresses clutter relative to conventional processing.
3. Preserves the target as the dominant peak of the adaptive RDM -- clutter
   suppression alone is not enough, since nulling the target too would still
   pass Test 2.
4. Builds a space-time steering vector that matches the array spatial steering
   vector and the temporal Doppler phase ramp.
...
# length in samples).  A strong target's compression sidelobes otherwise leak
# into the training data and STAP nulls the target itself (self-cancellation).
pulse_samples = len(waveform.pulse_sample)
n_guard = pulse_samples + 5
print(f"  Pulse length: {pulse_samples} samples -> using n_guard={n_guard}")
...
if len(clutter_bins) > 0:
    ...
    suppression_ok = suppression_db > 5
    ...
    print(
        f"  Clutter suppression:        {suppression_db:.1f} dB {'PASS' if suppression_ok else 'FAIL'}"
    )
else:
    print("  (not enough range bins for clutter measurement)")

# -- Test 3: Verify the target SURVIVES STAP (not just that clutter dropped) --
# Clutter suppression alone is not enough: a processor that nulls everything
# (target included) would also pass Test 2.  Confirm the target is still the
# dominant peak of the adaptive map, at the correct range-Doppler cell.
print("\n--- Test 3: Target preservation in the adaptive RDM ---")
adapt_mag = np.abs(adapt_clut)
adapt_peak = np.unravel_index(adapt_mag.argmax(), adapt_mag.shape)
adapt_peak_range = r_axis[adapt_peak[0]]
adapt_peak_rdot = rdot_axis[adapt_peak[1]]

peak_range_ok = abs(adapt_peak_range - tgt_range) < 2 * range_res
peak_rdot_ok = abs(adapt_peak_rdot - tgt_rdot) < 2 * rdot_res
target_preserved = peak_range_ok and peak_rdot_ok

# Target prominence above the residual floor (median of the adaptive map)
target_cell = (peak_idx[0], np.argmin(np.abs(rdot_axis - tgt_rdot)))
prominence_db = 20 * np.log10(adapt_mag[target_cell] / np.median(adapt_mag))
print(
    f"  Adaptive peak range:      {adapt_peak_range / 1e3:.1f} km (target {tgt_range / 1e3:.1f} km)"
)
print(f"  Adaptive peak range-rate: {adapt_peak_rdot:.1f} m/s (target {tgt_rdot:.1f} m/s)")
print(f"  Target prominence:        {prominence_db:.1f} dB above the residual floor")
print(f"  Target is dominant peak:  {'PASS' if target_preserved else 'FAIL'}")

# -- Test 4: Verify steering vector --
print("\n--- Test 4: Space-time steering vector ---")
...
# -- Summary --
all_pass = (
    range_ok and rdot_ok and suppression_ok and target_preserved and spatial_ok and temporal_ok
)
print("\n" + "=" * 60)
