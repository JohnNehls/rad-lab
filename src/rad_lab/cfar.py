...
def cfar_2d(
    ...
    that the probability of false alarm is *pfa* for complex Gaussian
    noise, whose power is exponentially distributed after the square-law
    detector.

    The threshold multiplier is exact only for CA; GOCA and SOCA reuse it
    as an approximation (their exact multipliers have no closed form).
    For the same multiplier, GOCA's max operation yields a false-alarm
    rate somewhat *below* the requested *pfa* and SOCA's min operation
    somewhat *above* it.
    ...
    Args:
        ...
        method: CFAR variant.  One of:
            ...
            - ``"GOCA"``: Greatest-of CA — uses the larger of the leading and
              lagging halves (split in the range dimension).  More robust at
              clutter edges, but masks targets when an interferer or clutter
              lies in either half.
            - ``"SOCA"``: Smallest-of CA — uses the smaller half.  Detects
              targets that CA/GOCA mask when one half is contaminated, at the
              cost of a much higher false alarm rate at clutter edges.
    ...
    # As N -> infinity this approaches -ln(Pfa), the fixed threshold of
    # detection.threshold_factor (known noise power).
    alpha = n_train * (pfa ** (-1.0 / n_train) - 1)

    # Pad by wrapping: both RDM axes are circular (ranges beyond the
    # unambiguous range and Dopplers beyond the PRF alias back in), so the
    # cells past each edge really are the cells from the opposite edge.
    ...
    for i in range(n_range):
        for j in range(n_doppler):
            ...
            if method == CfarType.CA:
                ...
                # Boolean indexing flattens the window row-major, so the first
                # half of train_cells is approximately the leading-in-range
                # cells and the second half the lagging-in-range (the cells at
                # the CUT's own range straddle the split).
